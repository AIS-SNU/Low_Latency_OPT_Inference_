...
class OptConfig:
    ...
    def model_bytes_dist(self, world_size):
        h = self.input_dim
        return 	2 * (self.num_hidden_layers * (
        # self-attention
        3 * h * (h + 1) // world_size + h * (h // world_size + 1) +
        # mlp
        h * (4 * h // world_size + 1) + h * 4 * (h + 1) // world_size +
        # layer norm
        h * 4) +
        # embedding
        self.vocab_size * (h // world_size + 1))
    def model_bytes(self):
        ...
        # embedding
        self.vocab_size * (h + 1))
    def cache_bytes_dist(self, batch_size, seq_len, world_size):
        return 2 * batch_size * seq_len * self.num_hidden_layers * self.input_dim * 2 // world_size
